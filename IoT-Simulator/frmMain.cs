    ...
    public partial class frmMain : Form
    {
        ...
        private async void btnRegistry_Click(object sender, EventArgs e)
        {
            ...
            if (objDevice != null)
            {
                ...
                // 指定裝置物件的資訊，並建立連線狀態
                deviceClient = DeviceClient.CreateFromConnectionString($"HostName={strIoTHubUrl};DeviceId={txtDeviceId.Text};SharedAccessKey={txtDeviceKey.Text}", Microsoft.Azure.Devices.Client.TransportType.Mqtt);

                // 放入傳送訊息用的內容
                MessageModel objMsg = new MessageModel()
                {
                    eventTime = IoT_Simulator.DateTimeExtensions.ToUnixTimestamp(DateTime.Now),
                    uid = new Random().Next(100, 140).ToString() + new Random().Next(100, 110).ToString(),
                    systemtype = "給排水_給水",
                    devtype = "揚水泵",
                    devname = "揚水泵-01",
                    unit = "0_10_Kg/cm2",
               ...
               // txtMessage.Text ="{\"DeviceId\":\"andy\",\"eqip\":\"andy\",\"SendDateTime\":\"10-12-2018 11:38:03\",\"value\":\"23\",\"status\":\"andy\"}";
            }
            else
            {
